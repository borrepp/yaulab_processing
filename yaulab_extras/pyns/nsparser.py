...
# .nev files
NEURALEV_FORMAT = b"<8s2BH4I8H32s256sI"
NEUEVWAV_FORMAT = b"<8sH2B2H2h2Bf6s"
NEUEVLBL_FORMAT = b"<8sh16s6s"
NEUEVFLT_FORMAT = b"<8sH2IHIIH2s"
DIGLABEL_FORMAT = b"<8s16sB7s"
NSASEXEV_FORMAT = b"<8s2BhBhBhBhBhBh6s"
...
def ParserFactory(filename):
    ...
    if file_type == b"NEURALEV":
        return NevParser(fid)
    elif file_type == b"NEURALSG":
        return Nsx21Parser(fid)
    elif file_type == b"NEURALCD" or file_type == b"NEUCDFLT":
...
class NevParser:
    ...
    def __init__(self, fid):
        ...
        self.n_data_packets = int((self.size - self.bytes_headers) / self.bytes_data_packet)
    ...
    def get_extended_header(self, header_index=None):
        ...
        elif header_type == b"NEUEVLBL".decode('utf-8'):
        ...
        elif header_type == b"NEUEVFLT".decode('utf-8'):
    ...
    def get_packet_headers(self):
        ...
        while remaining_packets > 0:
            ...
            bytes_to_read = int(packets_to_read*self.bytes_data_packet)
...
class Nsx21Parser:
    ...
    def get_analog_data(self, channel, start_index, index_count): 
        ...
        bin_count = 0
        for iBin in range(0, index_count):
...
#        self.scale = self.scale / (header.max_dig_value - header.min_dig_value)
    ...
    def get_basic_header(self):
        ...
        if not(tup[0] == b"NEURALCD" or tup[0] == b"NEUCDFLT"):
            raise NeuroshareError(NSReturnTypes.NS_BADFILE,
