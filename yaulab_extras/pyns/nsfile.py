...
#            nsx_files.reverse()
        ...
        for filename in file_list:
            ...
            if parser.file_type == b"NEURALEV":
                ...
                for (channel_index, header) in enumerate(parser.get_extended_headers()):
                    ...
                    units = header.units.split(b'\0')[0]
                    label = header.electrode_label.split(b'\0')[0]
    ...
    def _load_neuralev(self, file_data):
        ...
        for header in parser.get_extended_headers():
            ...
            if header.header_type == b"NEUEVWAV":
                ...
                if header.packet_id in list(entity_search.keys()):
                    entity_search[header.packet_id].label = header.label.split(b"\0")[0]
                else:
        ...
        if USE_MEM_CHECK:
            phymem = psutil.virtual_memory().free # physical memory in bytes
        ...
        for (electrode_id, label) in list(entity_labels.items()):
            try:
                # set the label for corresponding entity, label is NULL terminated
                entity_search[electrode_id].label = label.split(b"\0")[0]
            except KeyError:
        ...
        for (ipacket, packet_data) in enumerate(parser.get_packet_headers()):
            ...
            if packet_id == 0:
                if not unit in list(event_entities.keys()):
            ...
            # packet_id > 0 (corresponding to electrode_id) is a spike waveform event
                ...
                if not packet_id in list(neural_entities.keys()):
                    neural_entities[packet_id] = {}                    
                # unit_class = unitpacket.unit_class
                if not unit in list(neural_entities[packet_id].keys()):
    ...
    def get_time(self):
        ...
        time = datetime.datetime(info.time_year, info.time_month, info.time_day,
                                 ...
                                 int(info.time_millisec*1000),UTC())
        return time
    ...
    def get_file_info(self):
        ...
        for f in self._files:
            ...
            if header.header_type == b"NEURALEV":
                ...
                app_name = header.application.split(b"\0")[0]
                comment = header.comment.split(b"\0")[0]
